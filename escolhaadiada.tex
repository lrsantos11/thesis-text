...
utilizados atualmente~\cite{Wright:Primal-dual-interior-point:1997h}. Isso significa que de certa forma, nosso método generaliza os \ac{MPI} existentes.
...
%XXX: Escrever sobre a função de mérito do Mehrotra
...
Consideramos que a função de mérito para o próximo ponto, $\nextphi(\al,\mu,\sig) $ representa as propriedades (i-v) acima descritas. Com isso, alcança-se o objetivo de encontrar uma função de mérito que tenha boas propriedades matemáticas e que ao mesmo tempo dê uma medida adequada da qualidade solução.
...
Caso $\rho_L^0 = 0$, então encontramos um ponto inicial viável e o Algoritmo resolve o problema como um método factível o faz. Na prática, porém, essa situação é rara.

Usando a Definição \ref{def:residual-vector} e a Proposição \ref{prop:nu_k}, é possível rescrever  a vizinhança
de passo largo infactível $\Nset_{-\infty}(\ga,\be)$ para o  o sistema KKT escalado
como
\begin{equation}
\Nset_{-\infty}(\ga,\be) =
...
Portanto, para expressar tal problema de forma apropriada,  assim
como feito com $\nextphi$, vamos explicitar as desigualdades   \eqref{eq:symmetric-polynomials}
 para o próximo ponto. Veremos que estas podem ser calculadas
termos de  $(\al,\mu,\sig)$.
