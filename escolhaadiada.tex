...
A explicação de cada passo do Algoritmo \ref{alg:optimized-choice-of-parameters} será feita a seguir -- a menos dos passos \num{7} e \num{12}, através da explicitação do MEOP. Para dar conta disso e, portanto, dos objetivos propostos no presente capítulo,  são necessárias algumas transformações, as quais explanamos a seguir. 
...
problema são\footnote{Novamente, durante todo este capítulo, estamos utilizando a notação do produto de Haddamard e assim,  $uv$ indica o produto componente a componente de $u$ e $v$.}

\begin{subnumcases}{\label{eq:KKT-choice}}
Ax =b,\label{eq:KKT-fac-primal-choice}\\
A^Ty + z =c, \label{eq:KKT-fac-dual-choice}\\
xz =0,  \label{eq:KKT-complementar-choice}\\
...
r_C  &=xz. \label{eq:KKT-residuals-c}
\end{align} 
\end{subequations}
...
sejam  não-negativos -- veja Observação \ref{obs:Choice-of-merit-function} na página \pageref{obs:Choice-of-merit-function}.
...
xz =0,  \label{eq:ScaledKKT-complementar}\\
...
\eqref{eq:ScaledKKT}  -- é alcançada quando todos os resíduos forem nulos. De fato, para fins de implementação, diremos que a otimalidade foi alcançada  sempre que a norma de cada um desses resíduos for menor ou igual a uma  
...
xz =\mu e,  \label{eq:Homotopy-complementar}\\
...
\hat{x}\hat{z} &= (x + \Dex)(z+\Dez) = xz + x\Dez + z\Dex + \Dex\Dez \\
                ...
                \eqref{eq:affine-scaling-system-compl} }}  + z\Dex^c + x\Dez^c +\Dex\Dez  \\
...
e suas demonstrações são encontrados na sequência. %no Apêndice
...
linear da iteração $k+1$, dada no Teorema \ref{thm:next=residual}. É possível, generalizar tal relação usando a  proposição seguinte que trata tanto dos resíduos lineares de KKT, quanto dos resíduos do KKT escalado. 
...
Até agora, o método proposto resolve o sistema \ac{KKT} escalado \eqref{eq:ScaledKKT}
...
função de mérito usando $\al>0$. O teste da razão é sempre utilizado em cada iteração para garantir isso.
Por outro lado, para $\mu$ e $\sig$ os limitantes superiores $u_{2}$ e $u_{3}$ são escalares positivos que podem ou não ser diferentes para esses dois parâmetros.
  ...
  O problema \eqref{eq:pop-subproblem} é um um Problema de Otimização de Polinômios (POP) e, em geral, é difícil de ser resolvido.  Considere que o conjunto dos polinômios aproxima o conjunto de todas as funções contínuas em conjuntos compactos e, por isso, minimizar polinômios globalmente não é uma tarefa  trivial. 
  Com efeito, para alguns exemplares, um POP é NP-Hard \cite{Lasserre:2001fw,Laurent:2010kp}. 

  No entanto, em nosso caso, \textcite{VillasBoas2013:wn} resolvem um problema com características parecidas com \eqref{eq:pop-subproblem} em tempo computacional muito baixo, o que permite sua utilização em termos práticos. Um resumo dessa estratégia, que foi utilizada em nossa implementação, está no Capítulo~\ref{chap:implementa}.


 No presente capítulo estabelecemos as bases do Método de Escolha Otimizada de Parâmetros, as quais se encontram resumidas  no Algoritmo \ref{alg:optimized-choice-of-parameters}. Nos  capítulos que seguem  estabeleceremos os resultados teóricos dessa abordagem, garantindo convergência e polinomialidade do algoritmo,  bem como os resultados dos experimentos computacionais de nosso método. Mais uma vez ressaltamos que nossa abordagem é tratar o MEOP tanto do ponto de visto teórico, quanto do de implementação, garantindo que os resultados da análise teórica sejam estabelecidos para o algoritmo que está, de fato, sendo implementado.
