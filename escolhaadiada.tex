...
Este capítulo tem o objetivo de apresentar a principal contribuição deste trabalho, que é o desenvolvimento de uma função de mérito polinomial,  utilizada na implementação do método,  que combina três direções -- a direção afim-escala, a direção de centralização e a direção de correção --  escolhendo o peso de cada uma destas na direção final através da  otimização global da função de mérito. Além disso, escreve-se a vizinhança $\Nset_{\infty}$, ajustada para pontos infactíveis, em termos das mesmas variáveis do polinômio, tal que a otimização da função de mérito está sujeita a um conjunto de restrições que garantem que o próximo ponto continuará na vizinhança. 

\section{Um KKT escalado}


 
A fim de construir a função de mérito e suas restrições como polinômios, são necessárias algumas transformações, as quais explanamos a seguir. Considere novamente o par primal-dual na sua forma padrão,
i.e.,  
...
vezes de parâmetro de barreira -- e de $\sig$ -- que vai pesar a contribuição da correção --  sendo feita de maneira adiada, e
...
Para tanto, note que a aproximação dada em  \eqref{eq:aproximacao-direcao}
...
\Dew^{\sig},
\end{equation} 
resolvendo então dois sistemas lineares similares ao da Seção 
...
em uma fatoração de Cholesky da matriz $B^{-1}$ e seguido de três \emph{backsolves}.
...
Por conta das propriedades \eqref{it:obs-Choice-merit-rho-l} e \eqref{it:obs-Choice-merit-rho-C} da Observação \ref{obs:Choice-of-merit-function}, podemos rescrever
...
\nu_k = \frac{\norm{\rho_L^{k}}_p}{\norm{\rho_L^{0}}_p},
\]
e logo, pela definição de \emph{operador média}, vale
\begin{equation}
	\label{eq:nu_k}
...
O Algoritmo \ref{alg:optimized-choice-of-parameters} resume o método proposto neste capítulo.
...
\Comment{Assegure que  $(x^0,z^0)>0$.}
	\Repeat
		...
		\eqref{eq:De_C}.
		\State Calcule  $\nextphi(\al_{k},\mu_{k},\sig_{k})$ usando
		\eqref{eq:merit-function-al-mu-sig} e $\psi(\al_{k},\mu_{k},\sig_{k})$.
