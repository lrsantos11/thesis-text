...
\begin{equation}
	\label{eq:theta}
\theta(\al) =  \dfrac{\al\left[ \nu_k\dbvec{\rho_L}_0 + (1-\eta)\dbvec{\rho_C} - \al\left(\dbvec{L_{0,0}} + \eta\dbvec{\rho_C} \dbvec{L_{1,0}}
\right) \right]}{\nu_k\dbvec{\rho_L}_0 +
\dbvec{\rho_C}}.
\end{equation}
...
\begin{lema}\label{lemma:boundxz1}
	...
	\end{equation}
	em que $\nu_k$ é definido em \eqref{eq:nu_k+1} e $\bdxzstar$ é definido em \eqref{eq:norm-xz-star}.
\end{lema}
...
Como todas as componentes de $(\xk,\zk)$ e $(\xstar,\zstar)$ são não-negativas, vale $((\xk)^{T}\zstar + (\xstar)^{T}\zk) \geq 0$. Além disso, $(\xstar,\ystar,\zstar)$ é uma solução ótima, e por isso $(\xstar)^{T}\zstar = 0$. Usando essas observações e levando em conta que $\nu_k\in(0,1)$,  pode-se reorganizar  a equação \eqref{eq:boundxz1} como
\begin{equation}
...
\begin{equation}
\label{eq:normxz1}
	(\xk)^{T}\zzero + (\xzero)^{T}\zk = \sum_{i=1}^{n} \bdxzstar\abs{x_i} +\sum_{i=1}^{n} \bdxzstar\abs{z_i} = \bdxzstar\norm{x}_{1} + 
	\bdxzstar\norm{z}_{1} = \bdxzstar\norm{(\xk,\zk)}_{1},
\end{equation}
e que
\begin{equation}
...
\begin{align*}
	\bdxzstar\nu_k\norm{(x,z)}_{1} &\leq \nu_k^{2}n\dbvec{\rho_C^0} + n\dbvec{\rho_C} + \nu_k(1-\nu_k)(\bdxzstar2n\bdxzstar) \\ 
								&\leq \nu_k n\dbvec{\rho_C^0}+ n\dbvec{\rho_C} + \nu_k(2n\bdxzstar^{2}) \\ 
...
\begin{equation}
	\label{eq:DxDzaff5}
	\norm{(xz)^{1/2}} = \left(\sum_{i=1}^{n}x_iz_i\right)^{1/2} = ((\xk)^{T}z)^{1/2} = n^{1/2}\dbvec{\rho_C}^{1/2} \leq \frac{n}{\ga^{1/2}}\dbvec{\rho_C}^{1/2},
\end{equation}
...
% \[
% \theta(\al) =  \dfrac{\al\left[ \nu_k\dbvec{\rho_L}_0 + (1-\eta)\dbvec{\rho_C} - \al\left(\zeta + \chi
% \right) \right]}{\nu_k\dbvec{\rho_L}_0 +
% \dbvec{\rho_C}}.
% \]
...
% 	\[
% 		\nextal = \frac{\nu_k\dbvec{\rho_L}_0 + (1-\eta)\dbvec{\rho_C}}{2(\zeta + \chi)}.
% 	\]
...
% 	\[
% 		\nextal \geq  \frac{\nu_k\dbvec{\rho_L}_0 + (1-\eta)\dbvec{\rho_C}}{(\zeta + \chi)}.
% 	\]
% \end{enumerate}
% \end{proof}
