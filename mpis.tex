...
% \begin{enumerate}[$\Pset$1.]
%   \item O interior de $\Fset$ é não-vazio, isto é, existe algum ponto interior
%   factível.
%    \item Para quaisquer $(x,y,z)\in\Fset$ e $(\bar{x},\bar{y},\bar{z})\in\Fset$,
%  temos $(x-\bar{x})^T(z-\bar{z})\geq 0$.
%  
% \end{enumerate}
...
baseia-se em alguma variação do \acrodef{MPC}{Método Preditor-Corretor}
...
O \emph{método das múltiplas correções} deve-se a
...
\citet{Gertz:2003ji},	  em seu \emph{solver} \texttt{OOQP}, propõem uma
...
%XXX Fazer subseção sobre terminação finita
